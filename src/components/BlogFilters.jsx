...
export default function BlogFilters({ filters, onFilterChange, onReset }) {
  ...
  const handleIndustryToggle = (industryId) => {
    const current = filters.industry_id || []
    const updated = current.includes(industryId)
      ? current.filter(id => id !== industryId)
      : [...current, industryId]
    onFilterChange({ industry_id: updated.length > 0 ? updated : null })
  }

  const selectedIndustries = filters.industry_id || []
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
      <div className="flex flex-wrap gap-3">
        ...
        {/* Sort Order */}
        <Select value={filters.sort_order || 'desc'} onValueChange={(v) => onFilterChange({ sort_order: v })}>
